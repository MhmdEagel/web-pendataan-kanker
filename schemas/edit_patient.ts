...
export const editPatientSchema = z
  .object({
    no_register: z
      .string({
        error: (is) =>
          is.input === undefined
            ? "Nomor Register tidak boleh kosong"
            : "Input invalid",
      }),
    no_rm: z
      .string({
  ...
  .superRefine((data, ctx) => {
    if (data.fifth_survivor === "YA" && !data.fifth_survivor_tahun) {
      ctx.addIssue({
        code: "custom",
        path: ["fifth_survivor_year"], // penting biar error muncul di field ini
        message: "Tanggal lahir wajib diisi jika 5th = YA",
      });
    }
  });
